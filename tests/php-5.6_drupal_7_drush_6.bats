...
DOCKER_COMPOSE_FILE="${BATS_TEST_DIRNAME}/php-5.6_drupal_7_drush_6.yml"
...
@test "php-5.6: drupal 7: phpcs: phpcompatibility" {
  run docker exec "$(container)" /bin/su - root -mc "phpcs -i | grep 'PHPCompatibility'"

  [ "${status}" -eq 0 ]
}

@test "php-5.6: drupal 7: phpcs: drupal" {
