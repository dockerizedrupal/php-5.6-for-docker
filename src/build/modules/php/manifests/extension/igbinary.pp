class php::extension::igbinary {
  ...
  exec { 'phpize-5.6.1 igbinary':
    command => '/phpfarm/inst/bin/phpize-5.6.1',
    cwd => '/tmp/igbinary-1.2.1',
    require => Exec['tar xzf igbinary-1.2.1.tgz']
  }

  exec { '/bin/su - root -mc "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1 --enable-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/su - root -mc "cd /tmp/igbinary-1.2.1 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/igbinary-1.2.1 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1 --enable-igbinary"']
  }
