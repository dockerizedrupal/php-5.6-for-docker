class php::extension::apcu {
  ...
  exec { 'phpize-5.6.1 apcu':
    command => '/phpfarm/inst/bin/phpize-5.6.1',
    cwd => '/tmp/apcu-4.0.7',
    require => Exec['tar xzf apcu-4.0.7.tgz']
  }

  exec { '/bin/su - root -mc "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"':
    timeout => 0,
  ...
  exec { '/bin/su - root -mc "cd /tmp/apcu-4.0.7 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/apcu-4.0.7 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"']
  }
