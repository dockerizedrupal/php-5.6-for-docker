class php::extension::xdebug {
  ...
  exec { '/bin/su - root -mc "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"':
    timeout => 0,
  ...
  exec { '/bin/su - root -mc "cd /tmp/xdebug-2.2.6 && make"':
    timeout => 0,
    require => Exec['/bin/su - root -mc "cd /tmp/xdebug-2.2.6 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"']
  }
