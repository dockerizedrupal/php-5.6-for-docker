class php::extension::memcached {
  ...
  exec { '/bin/bash -l -c "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1 --enable-memcached-igbinary"':
    timeout => 0,
  ...
  exec { '/bin/bash -l -c "cd /tmp/memcached-2.2.0 && make"':
    timeout => 0,
    require => Exec['/bin/bash -l -c "cd /tmp/memcached-2.2.0 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1 --enable-memcached-igbinary"']
  }
