class build::php56::extensions::apd {
  ...
  bash_exec { 'cd /tmp/pecl-apd-master && phpize-5.6.19':
    require => Bash_exec['cd /tmp && unzip pecl-apd-master.zip']
  }

  bash_exec { 'cd /tmp/pecl-apd-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.6.19':
    timeout => 0,
    require => Bash_exec['cd /tmp/pecl-apd-master && phpize-5.6.19']
  }

  bash_exec { 'cd /tmp/pecl-apd-master && make':
    timeout => 0,
    require => Bash_exec['cd /tmp/pecl-apd-master && ./configure --with-php-config=/usr/local/src/phpfarm/inst/bin/php-config-5.6.19']
  }
