...
class php_extension_xdebug {
  ...
  exec { '/bin/bash -l -c "cd /tmp/xdebug-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"':
    require => Exec['phpize-5.6.1 xdebug']
  }

  exec { '/bin/bash -l -c "cd /tmp/xdebug-2.2.5 && make"':
    require => Exec['/bin/bash -l -c "cd /tmp/xdebug-2.2.5 && ./configure --with-php-config=/phpfarm/inst/bin/php-config-5.6.1"']
  }
...
class php {
  ...
  file { '/phpfarm/src/custom-options-5.6.1.sh':
    ...
    source => '/tmp/build/phpfarm/src/custom-options-5.6.1.sh',
    mode => 755,
    require => Class['phpfarm']
  }

  exec { '/phpfarm/src/compile.sh 5.6.1':
    timeout => 0,
  ...
  exec { 'rm -rf /phpfarm/src/php-5.6.1':
    path => ['/bin'],
    require => Exec['/phpfarm/src/compile.sh 5.6.1']
  }

  file { '/phpfarm/inst/php-5.6.1/etc/php-fpm.conf':
    ensure => present,
    source => '/tmp/build/phpfarm/inst/php-5.6.1/etc/php-fpm.conf',
    mode => 644,
    require => Exec['/phpfarm/src/compile.sh 5.6.1']
  }
  ...
  file { '/phpfarm/inst/php-5.6.1/lib/php.ini':
    ...
    source => '/tmp/build/phpfarm/inst/php-5.6.1/lib/php.ini',
    mode => 644,
    require => Exec['/phpfarm/src/compile.sh 5.6.1']
  }
